  ...
  return (
    <>
      ...
      <aside
        ...
        } ${isCollapsed ? 'w-[280px] lg:w-[76px]' : 'w-[280px] xl:w-[290px]'}`}
      >
        {/* Brand Header */}
        <div
          className={`border-b-2 border-[#242429] flex items-center transition-all duration-300 ${
            isCollapsed ? 'p-3.5 flex-col gap-3 justify-center' : 'p-6 justify-between'
          }`}
        >
          {isCollapsed ? (
            <>
              {/* Collapsed Brand Cube */}
        ...
        {/* Navigation Links */}
        <nav
          className={`flex-1 space-y-2 overflow-y-auto custom-scrollbar transition-all duration-300 ${
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center rounded-xl border-2 transition-all cursor-pointer ${
                  isCollapsed ? 'justify-center p-3' : 'gap-3.5 px-4 py-3 text-left'
                } ${
                  isActive
              ...
              >
                ...
                />
                {!isCollapsed && (
                  <span className={`font-display text-sm tracking-wide ${isActive ? 'font-black' : 'font-semibold'}`}>
                    {item.label}
                  </span>
                )}
              </button>
        ...
        {/* Footer Section */}
        <div
          className={`border-t-2 border-[#242429] bg-[#0D0D0F] transition-all duration-300 ${
        ...
        >
          <button
            ...
            className={`w-full flex items-center rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer border-2 ${
