...
export const StatsView: React.FC = () => {
  const { metrics, settings, playlists, progress } = useLearning();
    ...
    <div className="p-6 max-w-[1600px] mx-auto font-sans">
      ...
      <div className="bg-white border-3 border-black rounded-xl p-6 shadow-[5px_5px_0px_#000] mb-6">
        ...
        <p className="text-xs font-bold text-gray-600 mt-1">
          Detailed metrics, completion trajectories, and watch-time habits.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#FEF08A] border-3 border-black rounded-xl p-5 shadow-[4px_4px_0px_#000]">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-black uppercase text-black">Active Streak</span>
        ...
        <div className="bg-[#A7F3D0] border-3 border-black rounded-xl p-5 shadow-[4px_4px_0px_#000]">
          ...
          <p className="text-xs font-bold text-gray-700 mt-1">
            {metrics.overallProgress}% of catalog mastered
          </p>
        </div>
        ...
        <div className="bg-[#DDD6FE] border-3 border-black rounded-xl p-5 shadow-[4px_4px_0px_#000]">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-black uppercase text-black">Curriculums Active</span>
          ...
          <p className="text-xs font-bold text-gray-700 mt-1">
            {metrics.totalVideos} total videos tracked
          </p>
        </div>
      </div>
            ...
            <div className="space-y-4">
              {playlists.map((pl) => {
                ...
                const completed = pl.videos.filter(
                  (v) => progress[v.id]?.status === 'completed'
                ).length;
                ...
                return (
                  <div key={pl.id} className="space-y-1">
                    <div className="flex items-center justify-between text-xs font-black">
                      <span className="truncate max-w-[280px]">{pl.title}</span>
